...
/* Number of ticks until the next timer signal*/
int64_t next_alarm_clock_signal;
...
static bool alarm_clock_less(const struct list_elem *, const struct list_elem *,
                             void *);
...
static bool alarm_clock_less(const struct list_elem *a_, const struct list_elem *b_,
                             void *aux UNUSED)
{
  const struct alarm_clock *a = list_entry(a_, struct alarm_clock, elem);
  const struct alarm_clock *b = list_entry(b_, struct alarm_clock, elem);
...
timer_interrupt(struct intr_frame *args UNUSED)
{
  ...
  while (!list_empty(&alarm_clock_list) && ticks == next_alarm_clock_signal)
  {
    ...
    if (!list_empty(&alarm_clock_list))
    {
      ...
      next_alarm_clock_signal = next_alarm->signal_time;
    }
  }
